...
feature_names = ["cognitive_state", "vegetable", "age", "medicine", "MCC", "HGB", "NLR", "TC", "education"]
...
# medicine
medicine = st.selectbox("Medicine Use:", options=[0, 1], format_func=lambda x: '＜5' if x == 0 else '≥5')

# MCC
MCC = st.number_input("MCC Score:", min_value=0, max_value=10, value=0)

# HGB
HGB = st.number_input("HGB Level:", min_value=0, max_value=200, value=100)  

# NLR
NLR = st.number_input("NLR Ratio:", min_value=0, max_value=20, value=5)
...
# 准备输入特征
feature_values = [cognitive_state, vegetable, age, medicine, MCC, HGB, NLR,TC,education]
features = np.array([feature_values])

# 分离连续变量和分类变量
continuous_features = [age, MCC,HGB,NLR,TC]
categorical_features=[cognitive_state, vegetable,medicine,education]
