  ...
  # 将标准化前的原始数据存储在变量中
   ...
   # Display the SHAP force plot for the predicted class    
    if predicted_class == 1:
        ...
        shap_value_class = shap_values[:, :, 1]
    else:
    ...
    # 创建 SHAP force plot
    shap.force_plot(base_value, shap_value_class, original_feature_values, matplotlib=True)
    plt.gca().set_title(f'SHAP Force Plot for Class {predicted_class}')  # 可选：添加标题
    # 保存图像
    plt.savefig("shap_force_plot.png", bbox_inches='tight', dpi=1200)
 
    # 显示图像（仅在 streamlit 应用中有效）
